...
def create_user(
    *,
    ...
    mobile_number = get_unique_phone_number_for_user()
    data = {
...
def get_unique_phone_number_for_user():
    base = 5300
    numbers = []
    users = User.query.all()
    for user in users:
        numbers.append(user.mobile_number)
    print(f"ALL NUMBERS {numbers}")
    good_number = f"+1202555{base}"
    while good_number in (numbers):
        base = base + 1
        good_number = f"+1202867{base}"
    return good_number

...
def test_should_allow_api_call_if_under_day_limit_regardless_of_type(
    notify_api, mocker, restricted
):
    with notify_api.test_request_context():
        with notify_api.test_client() as client:
            mocker.patch("app.celery.provider_tasks.deliver_sms.apply_async")
            my_uuid = uuid.uuid4()
            my_email = f"{my_uuid}@fake.gov"
            service_name = f"Service {my_uuid}"
            user = create_user(mobile_number="+12025555999", email=my_email)
            ...
            data = {"to": user.mobile_number, "template": str(sms_template.id)}

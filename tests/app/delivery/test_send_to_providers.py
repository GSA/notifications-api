...
def test_should_call_send_sms_response_task_if_research_mode(
    ...
    send_sms_response.assert_called_once_with("sns", str(sample_notification.id))
...
def test_send_email_to_provider_should_call_research_mode_task_response_task_if_research_mode(
    ...
    send_email_response = mocker.patch(
        "app.delivery.send_to_providers.send_email_response"
    )
...
def test_should_not_update_notification_if_research_mode_on_exception(
    ...
    mocker.patch(
        "app.delivery.send_to_providers.send_sms_response", side_effect=Exception()
    )
    update_mock = mocker.patch(
        "app.delivery.send_to_providers.update_notification_to_sending"
    )
    sample_service.research_mode = True

...
def upgrade():
    ...
    conn.execute(
        text(
            "INSERT INTO INBOUND_NUMBERS (id, number, provider, active, created_at) VALUES(:inbound_number_id, "
            ":inbound_number, 'mmg', false, now())"
        ),
        input_params,
    )
...
def downgrade():
    conn = op.get_bind()
    input_params = {"sms_sender_id": SMS_SENDER_ID}
    conn.execute(
        text("delete from service_sms_senders where id = :sms_sender_id"), input_params
    )

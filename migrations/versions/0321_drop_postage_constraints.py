...
def upgrade():
    if environment not in ["live", "production"]:
        op.execute(
            "ALTER TABLE notifications DROP CONSTRAINT IF EXISTS chk_notifications_postage_null"
        )
        op.execute(
            "ALTER TABLE notification_history DROP CONSTRAINT IF EXISTS chk_notification_history_postage_null"
        )

    op.execute("ALTER TABLE templates DROP CONSTRAINT IF EXISTS chk_templates_postage")
    op.execute(
        "ALTER TABLE templates_history DROP CONSTRAINT IF EXISTS chk_templates_history_postage"
    )

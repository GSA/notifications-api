...
def upgrade():
    op.create_table(
        ...
        sa.Column(
            "broadcast_message_id", postgresql.UUID(as_uuid=True), nullable=False
        ),
    ...
    op.alter_column(
        "broadcast_message", "status", existing_type=sa.VARCHAR(), nullable=False
    )
    op.create_foreign_key(
        None, "broadcast_message", "broadcast_status_type", ["status"], ["name"]
    )


def downgrade():
    op.drop_constraint(
        "broadcast_message_status_fkey", "broadcast_message", type_="foreignkey"
    )
    op.alter_column(
        "broadcast_message", "status", existing_type=sa.VARCHAR(), nullable=True
    )
    op.alter_column(
        "broadcast_message",
        "areas",

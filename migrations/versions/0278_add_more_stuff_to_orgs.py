...
def upgrade():
    ...
    op.add_column(
        "organisation", sa.Column("agreement_signed", sa.Boolean(), nullable=True)
    )
    op.add_column(
        "organisation", sa.Column("agreement_signed_at", sa.DateTime(), nullable=True)
    )
    op.add_column(
        "organisation",
        sa.Column(
            "agreement_signed_by_id", postgresql.UUID(as_uuid=True), nullable=True
        ),
    )
    op.add_column(
        "organisation", sa.Column("agreement_signed_version", sa.Float(), nullable=True)
    )
...
def downgrade():
    op.drop_constraint(
        "fk_organisation_agreement_user_id", "organisation", type_="foreignkey"
    )
    ...
    op.drop_constraint(
        "fk_organisation_email_branding_id", "organisation", type_="foreignkey"
    )
    op.drop_column("organisation", "email_branding_id")

    op.drop_constraint(
        "fk_organisation_letter_branding_id", "organisation", type_="foreignkey"
    )
